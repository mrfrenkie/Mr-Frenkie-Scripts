...
local config_dir = r.GetResourcePath() .. "/Scripts/Mr. Frenkie Scripts/Audio Editing"
...
local function get_debug_report()
  if not r.FrenkieTransient_GetLastDebugReport then return "" end
  local s = r.FrenkieTransient_GetLastDebugReport()
  if not s then return "" end
  return tostring(s)
end

-- Debug report is always available via FrenkieTransient_GetLastDebugReport API
-- Debug console toggle is a separate REAPER action: "Transient Detector (Frenkie): Debug Console"
...
local SLIDER_GRAB = {
  { 0x3A8888FF, 0x50AAAAFF },  -- Low Cut: teal
  { 0x4A6A9AFF, 0x6088BBFF },  -- High Cut: steel blue
  { 0x6A4A8AFF, 0x8866AAFF },  -- Gain: purple
  { 0x8A4040FF, 0xAA5555FF },  -- Threshold: red
  { 0x4A7A4AFF, 0x66996BFF },  -- Sensitivity: green
  { 0x8A7030FF, 0xAA8844FF },  -- Retrig: amber
  { 0x8A5040FF, 0xAA6655FF },  -- Reduce: salmon
}
...
local debug_report = "" -- populated by FrenkieTransient_GetLastDebugReport
...
local function draw()
  ...
  if visible then
    ...
    r.ImGui_Spacing(ctx)
    if r.ImGui_Button(ctx, "Stretch Markers (Frenkie)") then
      write_json_config(cfg)
      trigger_extension()
      debug_report = get_debug_report()
    end
    r.ImGui_SameLine(ctx)
    if r.ImGui_Button(ctx, "Slice to Transients") then
      write_json_config(cfg)
      trigger_slice()
      debug_report = get_debug_report()
    end
      ...
      if not take or r.TakeIsMIDI(take) then
        ...
        if item_len_g > 0 then
          ...
          if peaks then
            ...
            -- Draw filtered waveform overlay (teal) — what the detector "sees"
